---
title: 'Lab 5 Solution: Metropolis-Hastings algorithm'
...
- proposal distribution and any accompanying variables
- components of the acceptance probability calculation
- starting value
...
one_iteration <- function(current, a, b){
  ...
  # Calculate components of acceptance probability 
  proposal_f <- dbeta(proposal, 2, 3) * dbinom(1, 2, proposal)
 proposal_q <- dbeta(proposal, a, b)
 current_f <- dbeta(current, 2, 3) * dbinom(1, 2, current)
...
beta_bin_mh_tour <- function(N, a, b){
  ...
  for(i in 1:N){
    # Simulate on iteration
    sim <- one_iteration(current = current, a = a, b = b)
